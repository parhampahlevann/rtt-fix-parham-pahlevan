...
get_current_dns() {
    ...
    if [ -z "$CURRENT_DNS" ] && command -v systemd-resolve &>/dev/null; then
        CURRENT_DNS=$(systemd-resolve --status | grep 'DNS Servers' | awk '{print $3}' | tr '\n' ' ')
    fi
    
    # Fallback to nmcli if available
    if [ -z "$CURRENT_DNS" ] && command -v nmcli &>/dev/null; then
        CURRENT_DNS=$(nmcli dev show | grep DNS | awk '{print $2}' | tr '\n' ' ')
    fi
}

# Disable DNS overwriting services
disable_dns_overwrite() {
    echo -e "${YELLOW}Disabling DNS overwriting services...${NC}"
    
    # Stop and disable systemd-resolved if exists
    if systemctl is-active --quiet systemd-resolved; then
        systemctl stop systemd-resolved
        systemctl disable systemd-resolved
        echo -e "${GREEN}Disabled systemd-resolved${NC}"
        
        # Restore original resolv.conf if it was a symlink
        if [ -L /etc/resolv.conf ]; then
            rm -f /etc/resolv.conf
            echo "nameserver 1.1.1.1" > /etc/resolv.conf
            echo "nameserver 8.8.8.8" >> /etc/resolv.conf
            echo -e "${GREEN}Restored standard resolv.conf${NC}"
        fi
    fi
    ...
    if systemctl is-active --quiet NetworkManager; then
        ...
        echo -e "${GREEN}Configured NetworkManager to ignore auto DNS${NC}"
    fi
    
    # Make resolv.conf immutable if regular file
    if [ -f /etc/resolv.conf ]; then
        chattr -i /etc/resolv.conf 2>/dev/null
        echo -e "${GREEN}Removed immutable attribute from /etc/resolv.conf${NC}"
    fi
    
    # Protect against dhclient
    if [ -f /etc/dhcp/dhclient.conf ]; then
        ...
        echo -e "${GREEN}Configured dhclient to preserve DNS${NC}"
    fi
    
    # Disable resolvconf if installed
    if [ -x /usr/bin/resolvconf ]; then
        echo "resolvconf=NO" > /etc/default/resolvconf
        echo -e "${GREEN}Disabled resolvconf updates${NC}"
    fi
}
...
set_dns_persistent() {
    ...
    # First disable any DNS overwriting services
    disable_dns_overwrite
...
EOF
        ...
        ifupdown)
            ...
            ;;
        *)
            echo -e "${YELLOW}Using direct resolv.conf method with multiple protections${NC}"
            ...
            echo -e "${GREEN}DNS set directly in /etc/resolv.conf with immutable flag${NC}"
            ;;
    esac
    ...
    # Verify DNS change
    verify_dns_change "${dns_servers[@]}"
}

# Verify DNS change was successful
verify_dns_change() {
    local expected_dns=("$@")
    local current_dns=($(get_current_dns | tr ' ' '\n'))
    
    # Compare DNS servers
    local match=true
    if [ ${#expected_dns[@]} -ne ${#current_dns[@]} ]; then
        match=false
    else
        for (( i=0; i<${#expected_dns[@]}; i++ )); do
            if [ "${expected_dns[i]}" != "${current_dns[i]}" ]; then
                match=false
                break
            fi
        done
    ...
    if $match; then
        ...
        echo -e "${BOLD_RED}Warning: DNS verification failed!${NC}"
        echo -e "Expected: ${expected_dns[*]}"
        echo -e "Current: ${current_dns[*]}"
        echo -e "${YELLOW}Trying fallback methods...${NC}"
        
        # Fallback 1: Use resolvectl if available
        if command -v resolvectl &>/dev/null; then
            for iface in "${ALL_INTERFACES[@]}"; do
                resolvectl dns "$iface" "${expected_dns[@]}"
            done
            echo -e "${YELLOW}Applied DNS via resolvectl${NC}"
        fi
        
        # Fallback 2: Use chattr to protect resolv.conf
        if [ -f /etc/resolv.conf ]; then
            chattr +i /etc/resolv.conf 2>/dev/null
            echo -e "${YELLOW}Made resolv.conf immutable${NC}"
        fi
        
        # Final verification
        get_current_dns
        echo -e "${YELLOW}Final DNS settings: ${CURRENT_DNS}${NC}"
    fi
}
...
reset_network() {
    ...
    if [[ -f "$SYSCTL_BACKUP" ]]; then
        ...
        # Reset DNS to default if backup exists
        if [ -f "$RESOLV_BACKUP" ]; then
            cp "$RESOLV_BACKUP" /etc/resolv.conf
            chattr -i /etc/resolv.conf 2>/dev/null
            echo -e "${GREEN}Restored original DNS settings${NC}"
        else
            DNS_SERVERS=("1.1.1.1" "8.8.8.8")
            set_dns_persistent "${DNS_SERVERS[@]}"
        fi
