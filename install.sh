...
configure_dns() {
    echo -e "\n${YELLOW}Automatic DNS Configuration${NC}"
    ...
    echo -e "3) Shecan: 178.22.122.100 185.51.200.2"
    echo -e "4) Electro: 78.157.42.100 78.157.42.101"
    echo -e "5) Custom: Enter your own DNS servers"
    echo -e ""
    ...
    # Detect and apply DNS configuration based on system type
    local dns_applied=false
    
    # Method 1: Systemd-Resolved (Ubuntu/Debian modern versions)
    if systemctl is-active --quiet systemd-resolved 2>/dev/null; then
        echo -e "${YELLOW}Detected systemd-resolved, applying DNS...${NC}"
        for dns in "${valid_dns_servers[@]}"; do
            resolvectl dns "$NETWORK_INTERFACE" "$dns" 2>/dev/null
        done
        systemctl restart systemd-resolved 2>/dev/null
        dns_applied=true
        echo -e "${GREEN}DNS applied via systemd-resolved${NC}"
    
    # Method 2: NetworkManager
    elif command -v nmcli >/dev/null 2>&1 && systemctl is-active --quiet NetworkManager 2>/dev/null; then
        echo -e "${YELLOW}Detected NetworkManager, applying DNS...${NC}"
        ...
        if [ -n "$con_name" ]; then
            ...
            dns_applied=true
            echo -e "${GREEN}DNS applied via NetworkManager${NC}"
        fi
    
    # Method 3: Netplan (Ubuntu 18.04+)
    elif [ -d /etc/netplan ]; then
        echo -e "${YELLOW}Detected Netplan, applying DNS...${NC}"
        local netplan_file=$(ls /etc/netplan/*.yaml 2>/dev/null | head -n1)
        if [ -f "$netplan_file" ]; then
            # Backup netplan file
            cp "$netplan_file" "$netplan_file.backup.$(date +%Y%m%d_%H%M%S)"
            ...
            dns_applied=true
            echo -e "${GREEN}DNS applied via Netplan${NC}"
        fi
    
    # Method 4: Traditional resolv.conf (fallback)
    else
        echo -e "${YELLOW}Using traditional resolv.conf method...${NC}"
        ...
        chattr +i /etc/resolv.conf 2>/dev/null
        dns_applied=true
        echo -e "${GREEN}DNS applied via resolv.conf${NC}"
    fi
    
    # Additional method for systems with /etc/resolv.conf.head
    if [ -f /etc/resolv.conf.head ]; then
        echo -e "${YELLOW}Updating resolv.conf.head...${NC}"
    ...
    # Restart networking services for changes to take effect
    ...
    if command -v dig >/dev/null 2>&1; then
        if dig +short google.com @${valid_dns_servers[0]} >/dev/null 2>&1; then
            ...
            echo -e "${YELLOW}⚠ DNS test failed, but configuration applied. Checking alternative...${NC}"
            # Try second DNS server
        ...
        echo -e "${YELLOW}⚠ dig command not available, skipping DNS test${NC}"
    fi
...
# Reset to default DNS - Fixed Version
reset_dns() {
    ...
    if systemctl is-active --quiet systemd-resolved 2>/dev/null; then
        ...
        local netplan_file=$(ls /etc/netplan/*.yaml 2>/dev/null | head -n1)
        if [ -f "$netplan_file" ]; then
            # Remove DNS settings from netplan
    ...
    # Fallback to default resolv.conf
