...
configure_edns_dns() {
    ...
    # Method 1: Direct interface configuration
    if ip link show edns0 >/dev/null 2>&1; then
        for dns in "${dns_servers[@]}"; do
            # Try to set DNS via dhclient if available
            if command -v dhclient >/dev/null 2>&1; then
                dhclient -r edns0 2>/dev/null
                dhclient -4 -pf /var/run/dhclient.edns0.pid -lf /var/lib/dhcp/dhclient.edns0.leases edns0
            fi
        done
    fi
    
    # Method 2: Create edns-specific resolv.conf
    mkdir -p /etc/edns
    cat > /etc/edns/resolv.conf <<EOL
# edns0 interface DNS configuration
# Generated by $SCRIPT_NAME

EOL
    for dns in "${dns_servers[@]}"; do
        echo "nameserver $dns" >> /etc/edns/resolv.conf
    done
    
    # Method 3: Systemd network configuration
...
configure_dns() {
    ...
    # Always update /etc/resolv.conf first
    update_resolv_conf "${valid_dns_servers[@]}"
    
    # Configure edns0 interface if present
    if ip link show edns0 >/dev/null 2>&1 || ip addr show | grep -q edns0; then
    ...
    # Method 3: Netplan configuration
    if [ -d /etc/netplan ]; then
    ...
    # Method 4: Debian/Ubuntu interfaces file
    ...
    # Method 5: CentOS/RHEL resolv.conf management
    if [ -f /etc/sysconfig/network-scripts/ifcfg-$NETWORK_INTERFACE ]; then
        echo -e "${YELLOW}Configuring CentOS/RHEL network scripts...${NC}"
        local ifcfg_file="/etc/sysconfig/network-scripts/ifcfg-$NETWORK_INTERFACE"
        cp "$ifcfg_file" "$ifcfg_file.backup.$(date +%Y%m%d_%H%M%S)"
...
reset_dns() {
    ...
    # Reset edns0 interface if present
    if ip link show edns0 >/dev/null 2>&1; then
...
show_dns() {
    ...
    # Show edns0 configuration if present
    if ip link show edns0 >/dev/null 2>&1 && [ -f /etc/edns/resolv.conf ]; then
        echo -e "${BOLD}edns0 resolv.conf:${NC}"
        cat /etc/edns/resolv.conf 2>/dev/null
        echo -e "----------------------------------------"
    fi
