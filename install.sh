...
SCRIPT_VERSION="8.4"  # Updated version for clarity
...
configure_edns_dns() {
    ...
    # Method 2: Systemd network configuration
...
configure_dns() {
    ...
    if systemctl is-active --quiet systemd-resolved 2>/dev/null; then
        ...
        echo -e "${YELLOW}systemd-resolved stopped to prevent conflicts${NC}"
    fi
    ...
    local interfaces=($(ip link | grep '^[0-9]' | awk '{print $2}' | sed 's/://'))
...
EOL
    ...
    if [ $test_passed -eq 1 ]; then
        ...
        echo -e "${YELLOW}⚠ DNS configuration applied but tests failed. Check connectivity.${NC}"
    fi
