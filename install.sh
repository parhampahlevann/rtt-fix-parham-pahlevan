...
# Check Dependencies
check_dependencies() {
    local deps=("iproute2" "net-tools" "iptables" "bc")
    local missing_deps=()
...
ping_mtu() {
    ...
    if [[ "$test_mtu" =~ ^[0-9]+$ && $test_mtu -ge 68 && $test_mtu -le 9000 ]]; then
        ...
        ping -M do -s $((test_mtu - 28)) -c 4 8.8.8.8 || echo -e "${RED}Ping failed!${NC}"
    else
...
reset_network() {
    ...
    # Reset DNS
    if systemctl is-active systemd-resolved >/dev/null 2>&1; then
        resolvectl set-dns "$NETWORK_INTERFACE" 8.8.8.8 8.8.4.4 2>/dev/null
        resolvectl flush-caches >/dev/null 2>&1
    else
        chattr -i /etc/resolv.conf 2>/dev/null
