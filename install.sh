...
configure_dns() {
    ...
    echo -e "${YELLOW}Removing existing /etc/resolv.conf...${NC}"
    rm -f /etc/resolv.conf
    
    # Create new resolv.conf
    echo -e "${YELLOW}Creating new DNS configuration...${NC}"
    
    # Show DNS suggestions
    echo -e "\n${BLUE}Suggested DNS Servers:${NC}"
    echo "1) Google DNS: 8.8.8.8, 8.8.4.4"
    echo "2) Cloudflare DNS: 1.1.1.1, 1.0.0.1"
    echo "3) Yandex DNS: 77.88.8.8, 77.88.8.1"
    echo "4) AliBaba DNS: 223.5.5.5, 223.6.6.6"
    echo "5) Custom DNS"
    
    read -p "Choose DNS provider (1-5): " dns_choice
    ...
    case $dns_choice in
        ...
        5)
            ...
            echo -e "${YELLOW}Example: 8.8.8.8 1.1.1.1 9.9.9.9${NC}"
            ...
            for d in "${new_dns[@]}"; do
                if validate_ip "$d"; then 
                    ...
                    echo -e "${RED}Invalid IP: $d - skipping${NC}"
                fi
            done
            
            [ ${#valid[@]} -eq 0 ] && { echo -e "${RED}No valid DNS servers entered${NC}"; return 1; }
            DNS_SERVERS=("${valid[@]}")
            ;;
        *)
            echo -e "${RED}Invalid choice, using Google DNS${NC}"
            DNS_SERVERS=("8.8.8.8" "8.8.4.4")
            ;;
    esac
    
    # Create new resolv.conf
    echo -e "\n${YELLOW}Creating /etc/resolv.conf with DNS servers:${NC}"
    for dns in "${DNS_SERVERS[@]}"; do
        echo -e "  ${GREEN}nameserver $dns${NC}"
    done
...
EOF
    ...
    # Add options for better DNS resolution
    cat >> /etc/resolv.conf <<EOF
...
EOF
    ...
    chattr +i /etc/resolv.conf 2>/dev/null && echo -e "${GREEN}DNS configuration locked (immutable)${NC}" || echo -e "${YELLOW}Warning: Could not make resolv.conf immutable${NC}"
    
    # Also configure NetworkManager if available
    if command -v nmcli >/dev/null 2>&1 && (systemctl is-active --quiet NetworkManager 2>/dev/null || pgrep NetworkManager >/dev/null); then
        ...
        if [ -n "$con_name" ]; then
            ...
            echo -e "${GREEN}NetworkManager DNS updated${NC}"
        fi
    fi
    
    # Disable other DNS services to prevent conflicts
    local conflicting_services=("systemd-resolved" "resolvconf" "dnsmasq" "unbound" "bind9" "named")
    for svc in "${conflicting_services[@]}"; do
        if systemctl is-active --quiet "$svc" 2>/dev/null; then
            systemctl stop "$svc" 2>/dev/null
            systemctl disable "$svc" 2>/dev/null
            echo -e "${YELLOW}Disabled conflicting service: $svc${NC}"
        fi
    done
    ...
    if command -v dig >/dev/null 2>&1; then
        dig google.com +short >/dev/null 2>&1 && echo -e "${GREEN}✓ DNS resolution working${NC}" || echo -e "${YELLOW}⚠ DNS resolution test failed${NC}"
    fi
    
    echo -e "\n${GREEN}✓ DNS configuration completed successfully${NC}"
    echo -e "${GREEN}✓ Configuration will persist after reboot${NC}"
